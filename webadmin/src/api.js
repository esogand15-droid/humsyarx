...
export const api = {
  ...
  caScheduleTemplates: (group) => req('/api/web-admin/schedule/templates' + (group ? `?group=${encodeURIComponent(group)}` : '')),
  caScheduleTemplatesBulk: (body) => req('/api/web-admin/schedule/templates/bulk', { method: 'POST', body }),
  caScheduleTemplatesClear: (group) => req('/api/web-admin/schedule/templates' + (group ? `?group=${encodeURIComponent(group)}` : ''), { method: 'DELETE' }),
  caScheduleTemplatesGenerate: (body) => req('/api/web-admin/schedule/templates/generate', { method: 'POST', body }),
  caScheduleTemplatesScan: (file, group) => { const fd = new FormData(); fd.append('file', file); return req('/api/web-admin/schedule/templates/scan' + (group ? `?group=${encodeURIComponent(group)}` : ''), { method: 'POST', form: fd }); },
  caScheduleTemplatesScanConfirm: (body) => req('/api/web-admin/schedule/templates/scan/confirm', { method: 'POST', body }),
  caScheduleExamsScan: (file) => { const fd = new FormData(); fd.append('file', file); return req('/api/web-admin/schedule/exams/scan', { method: 'POST', form: fd }); },
  caScheduleExamsScanConfirm: (body) => req('/api/web-admin/schedule/exams/scan/confirm', { method: 'POST', body }),
  ...
  gradeLessonTerm: (lesson) => req(`/api/web-admin/grades/lesson-term?lesson=${encodeURIComponent(lesson)}`),
...
export function errText(e) {
  ...
  const map = {
    ...
    intake_not_configured: 'برای نقش شما هنوز ورودی‌ای تنظیم نشده؛ از ادمین ارشد بخواهید ورودی‌تان را تعیین کند.',
